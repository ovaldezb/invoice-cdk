...
def handler(event, context):
    print(f"SW_USER_NAME: {SW_USER_NAME}")
    print(f"SW_USER_PASSWORD: {SW_USER_PASSWORD}")
    print(f"SW_URL: {SW_URL}")  
    try:
        ...
        if http_method == "POST":
            ...
            print(f"Factura Generada Response: {factura_generada.json()}")
            dom = xml.dom.minidom.parseString(factura_generada.json().get("data").get("cfdi"))
            ...
            return {
                "statusCode": 200,
                "headers": headers,
                "body": json.dumps(factura_generada.json())
            }
